...
/**
 * @defgroup zwave_command_class_color_switch_types Type definitions for attribute storage of the Binary Switch Command Class.
 * @ingroup zpc_attribute_store_command_classes_types
 * @brief Type definitions for the Binary Switch Command Class.
 *
 *
@startuml{attribute_store_color_switch_command_class.png} "Binary Switch data model" width=10cm
allow_mixing
...
#ifndef ZWAVE_COMMAND_CLASS_BINARY_SWITCH_TYPES_H
#define ZWAVE_COMMAND_CLASS_BINARY_SWITCH_TYPES_H

#include <stdint.h>

///> Binary Switch value. uint32_t
// Use uint32_t for legacy reason to match attribute store type. Should be uint8_t.
typedef uint32_t binary_switch_value_t;

///> Duration. uint32_t
// Use uint32_t for legacy reason to match attribute store type. Should be uint8_t.
typedef uint32_t binary_switch_duration_t;

#endif  //ZWAVE_COMMAND_CLASS_BINARY_SWITCH_TYPES_H
